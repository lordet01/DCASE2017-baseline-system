...
obj_num = length(Data);
for k = 1:iter_num
    % constratints zero probs for left-to-right model
    if k == 1
        p_start_Mask = (p_start > 0);
        A_Mask = (A > 0);
    end
    p_start = p_start .* p_start_Mask + ~p_start_Mask .* 1e-9;
    A = A .* A_Mask + ~A_Mask .* 1e-9;
    p_start = mk_stochastic(p_start);
    A = mk_stochastic(A);
    ...
    disp(['loglike at' ,num2str(k),'th iteratation: ',num2str(loglik)]);
    if (loglik-pre_ll<log(converge)) 
        break; 
    end
    pre_ll = loglik;
end
