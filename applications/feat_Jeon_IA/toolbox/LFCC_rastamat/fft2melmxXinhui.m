function [wts,binfrqs] = fft2melmxXinhui(nfft, sr, nfilts, width, minfrq, maxfrq, params, htkmel, constamp)
...
%
% 'params' inserted in the input arguments list
if nargin < 8;     htkmel = 0;     end
...
minmel = hz2mel(minfrq, htkmel, params);
maxmel = hz2mel(maxfrq, htkmel, params);
binfrqs = mel2hz(minmel+[0:(nfilts+1)]/(nfilts+1)*(maxmel-minmel), htkmel, params);
...
function z = hz2mel(f, htk, params)
%  z = hz2mel(f,htk)
...
if htk == 1
  ...
  f_sp = 200/3; % 66.66667;
  % brkfrq = 1000;
  brkpt  = (params.brkfrq - f_0)/f_sp;  % starting mel value for log region
  % logstep = exp(log(6.4)/27); % the magic 1.0711703 which is the ratio needed to get from 1000 Hz to 
                              % 6400 Hz in 27 steps, and is *almost* the ratio between 1000 Hz and the preceding 
                              % linear filter center at 933.33333 Hz (actually 1000/933.33333 = 1.07142857142857
                              % and  exp(log(6.4)/27) = 1.07117028749447)
logstep = exp(log(6400/params.brkfrq)/(42-brkpt));
  linpts = (f < params.brkfrq);

  z = 0*f;

  % fill in parts separately
  z(linpts) = (f(linpts) - f_0)/f_sp;
  z(~linpts) = brkpt+(log(f(~linpts)/params.brkfrq))./log(logstep);

end


function f = mel2hz(z, htk, params)
%   f = mel2hz(z, htk)
...
%  logstep = exp(log(6.4)/27); % the magic 1.0711703 which is the ratio needed to get from 1000 Hz to 6400 Hz in 27 steps, and is *almost* the ratio between 1000 Hz and the preceding linear filter center at 933.33333 Hz (actually 1000/933.33333 = 1.07142857142857 and  exp(log(6.4)/27) = 1.07117028749447)
  ...
  f(~linpts) = params.brkfrq*exp(log(logstep)*(z(~linpts)-brkpt));
