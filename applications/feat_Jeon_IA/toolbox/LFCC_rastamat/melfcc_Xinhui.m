function [cepstra,aspectrum,pspectrum] = melfcc_Xinhui(samples, sr, params,  varargin)
%
% params - Xinhui added
% 
...
aspectrum = audspecXinhui(pspectrum, sr, nbands, fbtype, minfreq, maxfreq, sumpower, bwidth, params);

% Copy from http://labrosa.ee.columbia.edu/matlab/rastamat/rastaplp.m
if params.dorasta ~= 0
    fprintf(1, 'RASTA processing !\n') ; 
  % put in log domain
  nl_aspectrum = log(aspectrum);

  % next do rasta filtering
  ras_nl_aspectrum = rastafilt_xinhui(nl_aspectrum);

  % do inverse log
  aspectrum = exp(ras_nl_aspectrum);

  % for Hacking
  % save('temp.mat', 'nl_aspectrum', 'ras_nl_aspectrum')
end
